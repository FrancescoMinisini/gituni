...
class  Prova3: public FunzioneVettorialeBase {
public:
  Prova3 (double alpha): _alpha(alpha) {};

  virtual arma::vec Eval(double t, const arma::vec & x) const {
        // x[0] = posizione, x[1] = velocità
        arma::vec dxdt(4);
        dxdt[0] = x[2];             // Derivata della posizione = velocità
        dxdt[1] = x[3]; 
        dxdt[2] = -x[3]/pow(x[0]*x[0]+x[1]*x[1],_alpha*0.5);  
        dxdt[3] = x[2]/pow(x[0]*x[0]+x[1]*x[1],_alpha*0.5);   // Derivata della velocità = accelerazione
        return dxdt;
  };
...
class  Prova32: public FunzioneVettorialeBase {
public:
  Prova32 (double alpha): _alpha(alpha) {};
  ...
  virtual arma::vec Eval(double t, const arma::vec & x) const {
        ...
        dxdt[0] = x[2];             // Derivata della posizione = velocità
        dxdt[1] = x[3]; 
        dxdt[2] = -1-x[3]/pow(x[0]*x[0]+x[1]*x[1],_alpha*0.5);  
        dxdt[3] = 1+x[2]/pow(x[0]*x[0]+x[1]*x[1],_alpha*0.5);   // Derivata della velocità = accelerazione
        return dxdt;
  };
...
class OscillatoreSmorzato : public OscillatoreArmonico {
public:
    OscillatoreSmorzato(double omega0 , double alpha)
        : OscillatoreArmonico(omega0), _alpha(alpha) {}
    ...
    virtual arma::vec Eval(double t, const arma::vec &x) const override {
        ...
        arma::vec dxdt(2);
        dxdt[0] = x[1]; // Derivata della posizione = velocità
        dxdt[1] = -_omega0 * _omega0 * x[0] - _alpha * x[1]; // Accelerazione
        return dxdt;
    }
