...
public:
    ...
    vector<Measure> CercaZeriMultipli(arma::vec inf, arma::vec sup,  const FunzioneBase& f ) {
        if(inf.size() != sup.size()) { throw std::runtime_error("La dimensione dei vettori non coincide");}
    vector<Measure> result;
    for (int i = 0; i < inf.size(); i++)
    {
        try
        {
        result.push_back(CercaZeri(inf[i], sup[i], f));
        }
...
private:
    Measure CercaZeriLogica(double xmin, double xmax, const FunzioneBase& f, double prec, unsigned int nmax) {
        ...
        if (sign.Eval(f.Eval(xmin))  * sign.Eval(f.Eval(xmax)) == 1) {
