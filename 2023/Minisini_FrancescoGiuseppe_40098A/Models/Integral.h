...
public:
  ...
  double GetPassoRunTime(double prec,  const FunzioneBase &f, double h_generico = 0.01){
    unsigned int nstep = (unsigned int)(_b-_a)/h_generico;
    double errore = ErroreRunTime(nstep, f);
    return pow(prec*h_generico*h_generico/errore,.5);
  }

  double ErroreRunTime(unsigned int nstep, const FunzioneBase &f) {
    Measure In = Integra(nstep, f);
    Measure I2n = Integra(2*nstep, f);
    return 4.*abs(In.GetValue()-I2n.GetValue())/3.;
  }
//Valuta se inserirlo in Integra
...
public:
  ...
  double ErroreRunTime(unsigned int nstep, const FunzioneBase &f) {
    Measure In = Integra(nstep, f);
    Measure I2n = Integra(2*nstep, f);
    return 16.*abs(In.GetValue()-I2n.GetValue())/15.;
  }
  double GetPassoRunTime(double prec,  const FunzioneBase &f, double h_generico = 0.01){
    unsigned int nstep = (unsigned int)(_b-_a)/h_generico;
    double errore = ErroreRunTime(nstep, f);
    return pow(prec*pow(h_generico,4)/errore,.25);
  }
  ...
  Measure Integra(unsigned int nstep, const FunzioneBase &f) override
  {
    ...
    result *= h / 3.0;
    return Measure(result, 0.);
  }
  
  Measure IntegraConErroreRunTime(unsigned int nstep, const FunzioneBase &f) 
  {
Measure integrale = Integra(nstep, f);
Measure result(integrale.GetValue(), ErroreRunTime(nstep,f));
return result;
}
...
public:
  ...
  double GetPassoRunTime(double prec,  const FunzioneBase &f, double h_generico = 0.01){
    unsigned int nstep = (unsigned int)(_b-_a)/h_generico;
    Measure I = Integra(nstep,f);
    double errore = I.GetSigma();
    return pow(prec*h_generico*h_generico/errore,.5);
  }
...
double StimaErroreIndottoSimpson(unsigned int seed, double a, double b, unsigned int niterations, unsigned int nstep, double sigma, const FunzioneBase &f);
double StimaErroreIndottoSimpson(unsigned int seed, double a, double b, unsigned int niterations, unsigned int nstep, double sigma, const FunzioneBase &f)
{
  RandomGen gen(seed);
  ...
  {
    Simpson myInt(a + gen.Unif(-sigma, sigma), b + gen.Unif(-sigma, sigma));
    Measure integrale = myInt.Integra(nstep, f);
    // cout<< 1000*(-3.473- integrale.GetValue())<<endl;
    integrals.push_back(integrale.GetValue());
  }
  return sqrt(CalcolaVarianza<double>(integrals));
}
#endif
