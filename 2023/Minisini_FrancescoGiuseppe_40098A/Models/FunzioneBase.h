...
virtual TGraph* Plot(string name, double initialValue = -5., double finalValue = 5., int n_points = 1000) const
{
    ...
    // Crea un TGraph con i dati calcolati
    TGraph* graph = new TGraph(n_points, &x_values[0], &y_values[0]);
...
public:
    ...
    TGraph* Plot(string name, double initialValue , double finalValue , int n_points) const override
    {   
        double vertex = GetVertex();
        return FunzioneBase::Plot(name, initialValue+ vertex, finalValue+vertex, n_points);
...
public:
    ...
    virtual double Eval(const arma::vec& x) const = 0;

    virtual double operator()(const arma::vec& x) const {
        return Eval(x);
    }
};

class FunzioneScalare : public FunzioneScalareBase {
public:
    FunzioneScalare(std::function<double(const arma::vec&)> f)
        : m_f(f) {}
    //ESEMPIO
    // FunzioneScalare myFunction([](const arma::vec &x) {
    //     return x[0] * x[0] + x[1];
    // });
    double Eval(const arma::vec& x) const override {
        return m_f(x);
    }

private:
    std::function<double(const arma::vec&)> m_f;
};
