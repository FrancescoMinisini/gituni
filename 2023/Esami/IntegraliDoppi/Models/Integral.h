...
public:
  ...
  double GetPasso(double prec,  const FunzioneBase &f, double h_generico = 0.01){
    unsigned int nstep = (unsigned int)(_b-_a)/h_generico;
    double errore = Errore(nstep, f);
    return pow(prec*h_generico*h_generico/errore,.5);
  }

  double Errore(unsigned int nstep, const FunzioneBase &f) {
...
public:
  ...
  double Errore(unsigned int nstep, const FunzioneBase &f) {
    Measure In = Integra(nstep, f);
    Measure I2n = Integra(2*nstep, f);
    return 16.*abs(In.GetValue()-I2n.GetValue())/5.;
  }
  double GetPasso(double prec,  const FunzioneBase &f, double h_generico = 0.01){
    unsigned int nstep = (unsigned int)(_b-_a)/h_generico;
    double errore = Errore(nstep, f);
